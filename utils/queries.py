...
def GET_EXTRATO_ZIG():
    df_extrato_zig = dataframe_query('''
      ...
      ''')
    df_extrato_zig = df_extrato_zig[df_extrato_zig['Casa'].isin(casas_validas)]
...
def GET_ZIG_FATURAMENTO():
    df_zig_faturamento = dataframe_query('''
      ...
      ''')
    df_zig_faturamento = df_zig_faturamento[df_zig_faturamento['Casa'].isin(casas_validas)]
...
def GET_PARCELAS_RECEIT_EXTR():
    ...
    ''')
    df_parc_receit_extr = df_parc_receit_extr[df_parc_receit_extr['Casa'].isin(casas_validas)]
...
def GET_CUSTOS_BLUEME_SEM_PARC():
    df_custos_blueme_sem_parcelam = dataframe_query('''
      ...
      ''')
    df_custos_blueme_sem_parcelam = df_custos_blueme_sem_parcelam[df_custos_blueme_sem_parcelam['Casa'].isin(casas_validas)]
...
def GET_CUSTOS_BLUEME_COM_PARC():
    ...
    ORDER BY 
      te.NOME_FANTASIA ASC, tc2.`DATA` DESC
    ''')
    df_custos_blueme_com_parcelam = df_custos_blueme_com_parcelam[df_custos_blueme_com_parcelam['Casa'].isin(casas_validas)]
...
def GET_EXTRATOS_BANCARIOS():
  df_extratos_bancarios = dataframe_query(''' 
    ...
    ''')
  df_extratos_bancarios = df_extratos_bancarios[df_extratos_bancarios['Casa'].isin(casas_validas)]
...
def GET_TESOURARIA():
    ...
    ''')
    df_tesouraria = df_tesouraria[df_tesouraria['Casa'].isin(casas_validas)]
...
def GET_AJUSTES():
    df_ajustes_conciliacao = dataframe_query('''
      ...
      ''')
    df_ajustes_conciliacao = df_ajustes_conciliacao[df_ajustes_conciliacao['Casa'].isin(casas_validas)]
...
def GET_BLOQUEIOS_JUDICIAIS():
    df_bloqueios_judiciais = dataframe_query('''
      ...
      ''')
    df_bloqueios_judiciais = df_bloqueios_judiciais[df_bloqueios_judiciais['Casa'].isin(casas_validas)]
...
def GET_EVENTOS():
    df_eventos = dataframe_query('''
      ...
      ''')
    df_eventos = df_eventos[df_eventos['Casa'].isin(casas_validas)]
    return df_eventos
